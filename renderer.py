        ...
        for r in range(BOARD_SIZE):
            for c in range(BOARD_SIZE):
                ...
                if board.grid[r, c] == HUMAN:
                    pygame.draw.circle(self.screen, COLOR_HUMAN, (x, y), CELL_SIZE // 2 - 6)
                    pygame.draw.circle(self.screen, (40, 40, 40), (x, y), CELL_SIZE // 2 - 9, 2)
                elif board.grid[r, c] == AI_PLAYER:
                    pygame.draw.circle(self.screen, COLOR_AI, (x, y), CELL_SIZE // 2 - 6)
                    pygame.draw.circle(self.screen, (200, 200, 200), (x, y), CELL_SIZE // 2 - 9, 2)

                if board.last_move and board.last_move[:2] == (r, c):
                    pygame.draw.circle(self.screen, COLOR_LAST_MOVE, (x, y), CELL_SIZE // 2 - 4, 5)

        if hovered and board.grid[hovered[0], hovered[1]] == EMPTY:
            hr, hc = hovered
            rect = pygame.Rect(hc * CELL_SIZE, hr * CELL_SIZE, CELL_SIZE, CELL_SIZE)
